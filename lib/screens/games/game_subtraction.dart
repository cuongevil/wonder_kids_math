...
class GameSubtraction extends StatefulWidget {
  const GameSubtraction({super.key});
  @override State<GameSubtraction> createState() => _GameSubtractionState();
}

class _GameSubtractionState extends State<GameSubtraction> {
  ...
  void _generateQuestion() {
    ...
    while (options.length < 4) { int wrong = rand.nextInt(10) + 1; if (!options.contains(wrong)) options.add(wrong); }
    options.shuffle();
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Game Trừ")),
      body: Stack(children:[
